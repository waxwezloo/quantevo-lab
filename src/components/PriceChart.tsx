...
export default function PriceChart({ candles, emaF, emaS, rsi, rsiLow, rsiHigh, trades, loading }: Props) {
  ...
  useEffect(() => {
    ...
    if (!canvas || size.w < 40 || !buckets) return;
  ...
  }, [size, buckets, hover, trades, m, rsiLow, rsiHigh]);

  const hj = hover >= 0 && hover < m ? hover : m - 1;
  const hc = buckets && hj >= 0 ? buckets.c[hj] : NaN;
  ...
  return (
    <div className="relative h-full w-full">
      ...
      {buckets && hj >= 0 && Number.isFinite(hc) ? (
