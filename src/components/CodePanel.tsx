...
function tokenizeLine(s: string): string {
  ...
  while ((m = TOKEN_RE.exec(s)) !== null) {
    ...
    else if (m[4]) out += `<span class="tk-kw">${m[4]}</span>${m[5]}<span class="${m[4] === "class" ? "tk-cls" : "tk-fn"}">${m[6]}</span>`;
    else if (m[7]) out += `<span class="tk-self">${m[7]}</span>`;
    else if (m[8]) out += `<span class="tk-kw">${m[8]}</span>`;
...
const PIPELINE = [
  ...
  { t: "Бэктест · JSON", d: "метрики · топ-5" },
];
