...
export default function FitnessChart({ history, running }: Props) {
  ...
  useEffect(() => {
    ...
    if (hover >= 0 && hover < g) {
      ...
      ctx.beginPath();
      ctx.arc(x, yV(history[hover].best), 3, 0, Math.PI * 2);
      ctx.fill();
    }
  }, [size, history, hover, g]);

  const hg = hover >= 0 && hover < g ? hover : g - 1;
    ...
    <div className="relative h-full w-full">
      ...
      {g > 0 && hg >= 0 ? (
        <div className="absolute top-1.5 left-3 qe-num text-[10.5px] text-mut bg-bg0/80 border border-line rounded-md px-2.5 py-1 pointer-events-none">
          <span className="text-dim">Поколение {history[hg].gen}</span>
          {" · best "}
          <span className="text-amber2">{history[hg].best.toFixed(2)}</span>
          {" · avg "}
          <span className="text-teal">{history[hg].avg.toFixed(2)}</span>
          {running && hg === g - 1 ? <span className="text-dim"> · идёт отбор…</span> : null}
        </div>
      ) : null}
    </div>
  );
