...
export default function EquityChart({ equity, bench, times, loading }: Props) {
  ...
  // защита от рассинхрона длин при смене таймфрейма/данных
  const bn = bench && equity ? Math.min(n, bench.length, Math.max(times.length, n)) : 0;
  ...
  }, [size, equity, bench, times, hover, n, bn]);
    ...
    <div className="relative h-full w-full">
      ...
      {equity && n > 0 && hi >= 0 && hi < n ? (
        <div className="absolute top-1.5 left-3 qe-num text-[10.5px] text-mut bg-bg0/80 border border-line rounded-md px-2.5 py-1 pointer-events-none">
          ...
          {bench && hi < bench.length && Number.isFinite(bench[hi]) ? (
            <>
              {" · B&H "}
